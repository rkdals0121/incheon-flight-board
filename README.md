...
- **서비스 워커(`sw.js`)는 항상 네트워크를 먼저 쓴다.** 캐시를 먼저 쓰도록 바꾸면 배포 직후
  구버전 JS·데이터가 남는 문제가 다시 생긴다. 캐시에 저장하는 방식을 바꿨다면 `sw.js` 의
  `CACHE` 이름(`icn-board-vN`)을 올려 이전 캐시를 지우게 할 것.
- "내 편" 고정은 사용자 브라우저(localStorage `icn-board:pins`)에만 저장된다.
